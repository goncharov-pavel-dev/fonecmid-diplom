...
Процедура РассчитатьДвиженияПоУдержаниям(); 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	МИНИМУМ(ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.ДатаНачала) КАК ДатаНачала,
	               |	МАКСИМУМ(ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.ДатаОкончания) КАК ДатаОкончания
	               |ИЗ
	...
	РезультатВТЗ = Запрос.Выполнить().Выгрузить();
	РезультатЗапроса = Запрос.Выполнить();	
	Выборка = РезультатЗапроса.Выбрать();

	
	Для Каждого Строка Из РезультатВТЗ Цикл 
