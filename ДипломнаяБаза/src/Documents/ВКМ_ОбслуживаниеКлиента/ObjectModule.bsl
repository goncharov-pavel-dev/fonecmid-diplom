...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//получить установленный процент оплаты от работ для сотрудника
	Запрос = Новый Запрос;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ДвижениеСотрудник = Движения.ВКМ_ВыполненныеСотрудникомРаботы.ДобавитьПриход();
			ДвижениеСотрудник.Период = ДатаПроведенияРабот;
			ДвижениеСотрудник.Сотрудник = Специалист;
			ДвижениеСотрудник.ЧасовКОплате = ВыполненныеРаботы.Итог("ЧасыКОплатеКлиенту");
			ДвижениеСотрудник.СуммаКОплате = ЗначениеЧасовойСтавки * ВыполненныеРаботы.Итог("ЧасыКОплатеКлиенту") * Выборка.ПроцентОтРабот / 100;
		КонецЦикла; 
		
	Иначе
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = СтрШаблон("Для сотрудника %1 не установлены условия оплаты", Специалист);
		Сообщение.Сообщить();
	КонецЕсли;
	...
	//Движение.СуммаКОплате = ЗначениеЧасовойСтавки * ВыполненныеРаботы.Итог("ЧасыКОплатеКлиенту") * Выборка.ПроцентОтРабот / 100;
